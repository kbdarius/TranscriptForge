...
Speaker identification is optional and local. Enable **Identify speakers locally** to analyze voice clusters, play representative samples, confirm names, and reuse confirmed speaker profiles in future recordings. The workflow has two review passes: the first captures obvious names, then confirmed profiles are used to resolve easy blanks and a shorter second dialog asks about the remaining uncertain voices before Whisper transcription starts. Very quiet or low-quality voice intervals are filtered before they become review samples, reducing empty and background-noise rows. Each remaining sample has its own **X** button so mixed or poor clips can be removed before profile learning. Existing names use a type-ahead picker: typing shows up to three matching names immediately; use Up/Down and Enter to choose one, click a visible match, or type a new name. A keyboard or mouse selection is committed into the field before the dialog continues. Names selected earlier in the same review are moved to the top of the later suggestions. Each review row also has a **Learn** checkbox; clear it when the label is useful for this transcript but the cluster should not contribute training data.

When a meeting is selected, speaker identification runs for that recording even if the general speaker-identification preference is off. Outlook invitees narrow automatic speaker matching to that meeting roster, and only those names are suggested initially. You can still type a guest name or choose **Unknown** for anyone not listed or not confidently matched. If the meeting is left unselected, the saved speaker-identification and Expected speakers settings continue to control the job as before.

For the scheduled-recording prompt, TranscriptForge reads Outlook appointments for the recording date. It uses a `YYYYMMDD_HHMMSS` or `YYYYMMDD-HHMMSS` timestamp in the recording filename when available, otherwise the file's modified time. Choosing a meeting also proposes its invitee count as an editable soft speaker limit. This is only a review priority: recognized profile matches and stronger/larger voice groups appear first, while extra distinct groups remain in an **Additional distinct voices** section. They are never merged or discarded just to satisfy the limit.

If an Outlook invitee name resembles a saved profile or a name in **Expected speakers**, TranscriptForge asks whether they refer to the same person. Confirming merges the saved embeddings and changes future speaker suggestions to Outlook's spelling. Declining keeps the names separate and remembers that decision. These local name decisions travel with speaker profiles in a portable configuration package.
...
When Outlook Classic is installed, the optional **Today's Outlook meeting** dropdown reads appointments from the default local calendar for the current day in the main window or for the recording date in the scheduled-recording prompt. No Microsoft Graph permission or cloud connection is needed. Select a meeting to use its organizer and invitees for speaker matching before speaker analysis begins. In the main window, TranscriptForge also uses the meeting title to recall the last output folder and filename base used for that recurring meeting, then applies the recording date (for example, `SW Daily Standup-20260922.md`). The local mapping is `%LOCALAPPDATA%\LocalAudioTranscriber\meeting-output-settings.json`; it is intentionally not included in portable configuration packages because output paths differ by computer.
