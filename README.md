...
When Outlook Classic is installed, the optional **Today's Outlook meeting** dropdown reads only appointments from the default local calendar for the current day—no Microsoft Graph permission or cloud connection is needed. The meeting list loads before transcription is allowed to start. Select a meeting to add its organizer and invitees as possible speakers before speaker analysis begins. TranscriptForge also uses the meeting title to recall the last output folder and filename base used for that recurring meeting, then applies the recording date (for example, `SW Daily Standup-20260922.md`). The local mapping is `%LOCALAPPDATA%\LocalAudioTranscriber\meeting-output-settings.json`; it is intentionally not included in portable configuration packages because output paths differ by computer.
